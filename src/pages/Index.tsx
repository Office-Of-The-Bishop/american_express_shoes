...
// ✅ Main Page
const Index = () => {
  const { addToCart, cart, saveCart } = useCart();
  ...
  const handleAddToCart = async (product: Product, quantity = 1) => {
    ...
    await axios.post(url, { product_id: product.id, quantity }).then((res) => {
            ...
            <div className="hidden xl:flex items-center gap-3 flex-1 justify-center text-sm font-medium">
              {[...blackTabs].map((tab) => (
                <a
                  ...
                  className={`px-2 py-1 rounded transition-all duration-150 ${activeTab === tab ? "underline underline-offset-4 font-semibold" : ""
